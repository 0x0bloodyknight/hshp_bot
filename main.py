...
def forward_to_nsfw(message):
    try:
        ...
        bot.reply_to(message, text="ТИ ЛЮДИНА ЧИ КОМПЮТОР?")


@bot.message_handler(
    commands=["nsfw"], func=lambda message: message.chat.id == config.hshp_id
)
def forward_to_nsfw_handler(message):
    forward_to_nsfw_thread = threading.Thread(target=forward_to_nsfw, args=(message,))
...
@bot.message_handler(
    commands=["purge_nsfw"], func=lambda message: message.chat.id == config.hshp_id
)
...
def help_message_handler(message):
    help_message_thread = threading.Thread(target=help_message, args=(message,))
